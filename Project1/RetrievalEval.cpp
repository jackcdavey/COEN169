		     ...
		     Index *ind)
{
  /*!!!!! Implement raw TF and IDF weighting scheme !!!!!*/
	double a = (log(ind->docCount()/ind->docCount(termID)));
	return a * docTermFreq * qryTermWeight;
}
		     ...
		     Index *ind)
{
  /*!!!!! Implement log TF and IDF weighting scheme !!!!!*/
	double a = (log(docTermFreq)+1);
	double b = (log(ind->docCount()/ind->docCount(termID)));
	return a * b * qryTermWeight;
}
		     ...
		     Index *ind)
{
  /*!!!!! Implement Okapi weighting scheme !!!!!*/
	double a = docTermFreq/(docTermFreq+0.5+(1.5*(ind->docLength(docID)/ind->docLengthAvg())));
	double b = log((ind->docCount()-ind->docCount(termID)+0.5)/(ind->docCount(termID)+0.5));
	double c = (8+qryTermWeight)/(7+qryTermWeight);
	return a * b * c;
}
